 ...
 */
function DiagramEditor(elt, config, ui, done, initialized, urlParams) {
    this.element = elt;
    ...
    this.handleMessageEvent = function (evt) {
        if (self.frame != null && evt.source == self.frame.contentWindow &&
            ...
            try {
                ...
                if (msg != null) {
                    console.log(msg, "data", self.getData());
...
DiagramEditor.editElement = function (elt, config, ui, done, urlParams) {
    if (!elt.diagramEditorStarting) {
        ...
        return new DiagramEditor(elt, config, ui, done, function () {
...
DiagramEditor.prototype.element = null;
...
DiagramEditor.prototype.editElement = function (elem) {
    ...
    if(src.startsWith("https://")){
        $.ajax({
            type: "get",
            url: "/api/v1/image?key="+this.element.getAttribute("alt"),
            async: false,
